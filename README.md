...
- frontend format/lint/typecheck, 36 Vitest tests и production/PWA build;
- Playwright: 8 тестов прошли и 2 project-specific skip ожидаемы; оба проекта используют Chromium/Chromium-based Pixel 7 и `VITE_DEMO_MODE=true`; дополнительно выполнен ручной authenticated UI/API sweep на real backend для desktop и 13 mobile routes;
- OpenAPI route inventory `66/48`, Redocly без ошибок/предупреждений, генерация 4,140-строчной TypeScript schema и последующие frontend lint/typecheck;
