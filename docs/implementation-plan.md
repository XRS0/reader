...
| Dictionary                     | CRUD/search/filter/status/delete/restore, ручное добавление, перевод и отдельное значение/описание, одноязычные записи без перевода, dedupe, occurrences, reader context, JSON export | PostgreSQL dedupe/occurrence/ownership и definition-only integration; real API acceptance; desktop/mobile E2E и component contracts                    | export ограничен первыми 100 entries; нет chunked/full background export в `user-exports`                                              |
...
| Frontend и PWA                 | auth/library/book/reader/dictionary/statistics UI, восстановление scroll/page позиции и визуальная линия окончания чтения, custom book covers, custom whole-app palette, simplified sidebar, command palette, preferences, demo/real transports, PWA/offline queue | format/lint/types; 42 Vitest tests; production/PWA build; 14 desktop/mobile Chromium demo E2E; отдельный browser acceptance восстановления scroll и paged позиции; authenticated real-backend API/UI sweep | offline reconnect и WebKit/Firefox не доказаны end-to-end                                                                              |
