...
| OpenAPI 3.1                    | 66 операций на 48 paths, единые errors/security/schemas/examples                                                                                                       | Gin/OpenAPI inventory совпадает; Redocly 2.39.0: 0 errors/0 warnings; heartbeat schema уточнена; TypeScript schema 4,140 строк; frontend lint/typecheck PASS | locator representation требует унификации; contract diff должен оставаться CI gate                                                     |
...
| Books и processing             | upload, immutable original, TXT/FB2/EPUB parsers, XHTML-aware sanitizer, NCX/EPUB3 TOC, worker jobs, deterministic chapter IDs, reprocess, download/assets             | parser tests; corrupt/ZIP limits; real TXT/FB2/EPUB acceptance; пользовательский EPUB переобработан без escaped markup; durable chapter references remapped  | reprocess заменяет пользовательский title извлечённым из файла; adversarial fuzz corpus и длительный soak/load остаются hardening      |
...
| Statistics                     | overview, daily/weekly/monthly, books, sessions, streak, dictionary, repeatable rebuild                                                                                | PostgreSQL aggregates/timezone/recompute integration; frontend views/build                                                                                   | median и translation count пока `0`; cross-midnight split отсутствует                                                                  |
...
| Frontend и PWA                 | auth/library/book/reader/dictionary/statistics UI, custom whole-app palette, simplified sidebar, command palette, preferences, demo/real transports, PWA/offline queue | format/lint/types; 36 Vitest tests; production/PWA build; 8 Chromium demo E2E; authenticated desktop/mobile real-backend UI sweep                            | 23.56% statement coverage; offline reconnect и WebKit/Firefox не доказаны end-to-end                                                   |
...
- [x] Daily/book/session statistics, timezone grouping и repeatable rebuild.
...
- [x] Desktop/mobile UI surfaces для reader, dictionary, notes и statistics.
...
| frontend `test:coverage`                                    | PASS                   | 16 files / 36 tests; 23.56% statements, 22.01% branches, 17.46% functions, 24.15% lines                                                                                              |
| frontend `build`                                            | PASS                   | 1,990 modules; main 384.37 kB / 118.47 kB gzip; Reader 56.78 kB / 19.59 kB gzip; PWA 38 entries / 686.32 KiB                                                                         |
| frontend `test:e2e`                                         | PASS с ожидаемыми skip | 8 passed + 2 project-specific intentional skips; Chromium; demo transport                                                                                                            |
| `scripts/check-openapi-routes.py`                           | PASS                   | 66 operations / 48 paths совпадают с Gin                                                                                                                                             |
...
- Frontend coverage низкое по абсолютной доле; 36 tests фокусируются на critical schemas/stores/offline queue/sidebar/tabs/menu/translation/EPUB-rendering primitives.
